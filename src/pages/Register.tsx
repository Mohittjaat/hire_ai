...
import { registerHR } from '../services/api';
...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('currentHRName', formData.fullName);
      localStorage.setItem('currentHRCompany', formData.company);
    ...
    <div className="min-h-screen bg-[#0a0c14] flex items-center justify-center p-6 font-sans text-white">
      <div className="w-full max-w-4xl bg-[#111622] rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col md:flex-row-reverse border border-slate-800">
        ...
        <div className="md:w-2/5 bg-gradient-to-br from-indigo-600 to-violet-700 p-12 text-white flex flex-col justify-center items-center text-center relative">
          <h2 className="text-3xl font-black mb-4 relative z-10 uppercase tracking-tight">Hello, HR!</h2>
          ...
          <Link to="/login" className="border-2 border-white/50 px-8 py-3 rounded-xl font-bold hover:bg-white hover:text-indigo-600 transition-all relative z-10">
            LOG IN
          </Link>
        </div>
        ...
        <div className="md:w-3/5 p-12 bg-[#111622]">
          <div className="max-w-sm mx-auto">
            ...
            {error && (
              <div className="mb-4 p-3 bg-red-500/10 border border-red-500/50 rounded-lg text-red-500 text-xs font-bold">
                {error}
              </div>
            )}
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div className="relative">
                ...
                <input type="email" name="email" placeholder="Work Email" required onChange={handleChange}
                  className="w-full bg-[#0a0c14] border border-slate-800 rounded-xl py-3.5 pl-12 pr-4 text-white focus:border-indigo-500 outline-none transition-all" />
              </div>

              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
                <input type="password" name="password" placeholder="Password (min 8 characters)" required minLength={8} onChange={handleChange}
                  className="w-full bg-[#0a0c14] border border-slate-800 rounded-xl py-3.5 pl-12 pr-4 text-white focus:border-indigo-500 outline-none transition-all" />
              </div>

              <button type="submit" disabled={loading}
                className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-700 py-4 rounded-xl font-bold shadow-xl shadow-indigo-900/20 transition-all flex items-center justify-center gap-2 mt-4 text-white">
